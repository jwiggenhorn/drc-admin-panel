...
export function exportAsCSV(participantDataList, studyTitle) {
  ...
  let maxDataLength = 0
  let numControls = 0 // TODO: change to use profile instead of calculating (participant could have never touched control)
  participantDataList.forEach(participantData => {
    numControls = 0
    for (const [k, v] of Object.entries(participantData)) {
      if (Array.isArray(v) && v.length > 0) {
        if (v.length > maxDataLength) maxDataLength = v.length;
        numControls++
      }
    }
  })
